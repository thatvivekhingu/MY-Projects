...
def recommend_recipe(ingredients, diet="", course="", region="", top_k=5):
    try:
        ...
        for i in top_indices:
            ...
            try:
                gemini_response = model.generate_content(f"Generate a realistic image description of the Indian recipe: {recipe_name}")
                if gemini_response.text:
                    recipe['image_prompt'] = gemini_response.text
            except Exception as gemini_error:
                recipe['image_prompt'] = f"Image generation error: {gemini_error}"
